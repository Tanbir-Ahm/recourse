...
CRITICAL: this chat feature does NOT cover every Indian criminal law. A question can genuinely be about "arrest" or "cognizable/non-cognizable" and still be out of this CHAT feature's scope, if the specific offence involved is governed by a law neither this chat feature nor the wider tool covers (e.g. POCSO, UAPA, a Dowry Prohibition Act-SPECIFIC offence, company/tax law offences). Do NOT classify a question as in_scope just because it uses words like "arrest", "cognizable", "police", or "FIR" -- check whether the SPECIFIC OFFENCE NAMED (if any) is actually a BNS/BNSS offence, a bank-freeze/cheque-bounce/domestic-violence/NDPS matter (see below), or a procedural question with no specific other-law offence attached.

This chat feature ALSO now has real, curated coverage (added 2026-09-25) of the Narcotic Drugs and Psychotropic Substances Act, 1985 (NDPS) -- an arrest or question about a drug offence: what is actually prohibited (Section 8), cannabis/ganja/charas (Section 20), "manufactured drugs" like heroin/morphine/cocaine (Section 21), consumption for personal use as a lighter offence than possession/sale (Section 27), bail (Section 37 -- the harsh "twin conditions" test that applies to commercial-quantity and a few other offences), being stopped and searched/arrested in a public place (Section 43), and the right to be taken before a Gazetted Officer or Magistrate before a personal search (Section 50). A question specifically about an NDPS/drug arrest, search, seizure, or bail (being caught with drugs, a family member arrested for possession/sale/consumption of a narcotic or psychotropic substance, questions about "small"/"commercial" quantity, whether Section 50's search right was followed) is "covered_elsewhere_in_tool" with redirect_domain "ndps" -- NOT adjacent_uncovered. This is DIFFERENT from POCSO, UAPA, or a purely civil dispute with no drug-related arrest/FIR involved, which remain adjacent_uncovered.
...
IMPORTANT -- sexual harassment and assault/criminal force intending to outrage a woman's modesty are ALSO in_scope, as real BNS offences (added 2026-09-22, found via a confirmed real failure): Section 75 (unwelcome physical contact/advances, a demand for sexual favours, showing pornography, or sexually coloured remarks) and Section 74 (assault or criminal force with intent to outrage modesty, e.g. groping/molestation). An arrest or FIR describing inappropriate touching, unwelcome advances, or harassing comments/behaviour towards a woman is in_scope for this reason, REGARDLESS of where it happened (a workplace, in public, anywhere else) -- do NOT classify this as adjacent_uncovered just because the word "workplace" appears, or because a separate, specialized law also exists for that context. CONFIRMED REAL FAILURE: "a colleague touched her inappropriately and made comments that made her uncomfortable... he's been arrested" was wrongly classified adjacent_uncovered, reasoning it "typically falls under POCSO or the Sexual Harassment of Women at Workplace Act" -- neither applies here. Two specific laws ARE genuinely different from this and remain adjacent_uncovered, but ONLY when they actually, specifically apply: POCSO (only when the person harassed is a CHILD -- never assume this from "workplace" alone; a colleague is an adult unless stated otherwise), and the Sexual Harassment of Women at Workplace Act, 2013 (POSH Act) -- a CIVIL, non-arrest, internal-complaints-committee process, applicable only when the question is about THAT civil process itself (filing an internal complaint, an Internal Committee inquiry) with NO arrest or police complaint described anywhere. An actual ARREST or FIR for this kind of conduct is a criminal matter under BNS 74/75, not the POSH Act, regardless of it happening at work.
...
If (and only if) the category is "covered_elsewhere_in_tool", also decide which specific domain: "freeze" (bank account freezing), "cheque_bounce" (Section 138 NI Act cheque dishonour), "domestic_violence" (PWDVA 2005), or "ndps" (NDPS Act 1985). For every other category, redirect_domain must be null.

Respond with ONLY a JSON object, no other text:
{{"category": "in_scope" | "covered_elsewhere_in_tool" | "adjacent_uncovered" | "unrelated", "redirect_domain": "freeze" | "cheque_bounce" | "domestic_violence" | "ndps" | null, "reasoning": "one short sentence"}}
...
def classify_scope(question):
    ...
    for attempt in range(2):
        try:
            ...
            if category != "covered_elsewhere_in_tool" or redirect_domain not in ("freeze", "cheque_bounce", "domestic_violence", "ndps"):
...
- When the material below includes a section that DEFINES a term another section's offence depends on (e.g. what actually counts as "grievous" hurt, or any other element a more serious classification turns on), treat that definition as a real test to check the facts against -- never assume a fact pattern satisfies it just because the injury or conduct described SOUNDS serious. A dramatic-sounding detail (a "deep cut", a scary-sounding word) is not the same as meeting the law's own specific definition. If the facts given are genuinely borderline or insufficient to tell which classification applies, say so plainly and name both the milder and the more serious possibility -- do not default to the more serious one just because it was mentioned first or seems safer to assume.
...
_INLINE_DOMAIN_CONFIG = {
    ...
    "ndps": {
        "override_import": ("ndps_doctrine_map", "get_ndps_override"),
        "closing_line": (
            "What you can do next: a lawyer or your nearest District Legal "
            "Services Authority (free) can look at the actual quantity involved, "
            "whether the Section 50 search right was followed, and prepare a "
            "bail application -- these details decide everything under this law "
            "and need someone who can see the actual case papers."
        ),
    },
}
